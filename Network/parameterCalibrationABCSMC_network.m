...
% pre-calculate travel distance
if samplingStrategy == 2 && configID == 41
    travel_distances(1) = linkMap(1).lengthInMiles + linkMap(3).lengthInMiles + sensorMetaDataMap(400739).offsetMiles;
    travel_distances(2) = linkMap(1).lengthInMiles + linkMap(3).lengthInMiles + linkMap(5).lengthInMiles + sensorMetaDataMap(400363).offsetMiles;
end
...
for stage = 1 : numStages  % iterate stages
    ...
    if stage == 1
        ...
        if samplingStrategy == 2
            % calculate travel times for sensors
            travel_times = calculateTravelTime(ACCEPTED_POP, configID, linkMap, sensorMetaDataMap);
            travel_times_new = cutSize(travel_times);
            mean_travelTimes = mean(travel_times_new,1);
            var_travelTimes = expectAR * var(travel_times,0,1);
        else
            mean_travelTimes = 0;
            var_travelTimes = 0;
            travel_times_new = 0;
        ...
        [ACCEPTED_POP, weights, ar, REJECTED_POP, errorCollectionForStage, thresholdVector, criteriaForStage] = ABC_SMC_stage2AndLater2_type2_network(measConfigID, configID, samplingSize, criteria,...
            ...
            sensorDataMatrix, nodeMap, errorCollectionForStage, ROUND_SAMPLES, occuDataMatrix_source,...
            occuDataMatrix_sink, mean_travelTimes, var_travelTimes, travel_times_new);
        
        if samplingStrategy == 2
            % calculate travel times for sensors
            travel_times = calculateTravelTime(ACCEPTED_POP, configID, linkMap, sensorMetaDataMap);
            travel_times = cutSize(travel_times);
            mean_travelTimes = mean(travel_times,1);
            var_travelTimes = var(travel_times,0,1);
        end
